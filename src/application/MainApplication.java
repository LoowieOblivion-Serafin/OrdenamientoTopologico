            ...
            try {
                ...
                for (Course course : courses) {
                    for (String prereq : course.getPrerequisites()) {
                        // Normalizamos de nuevo por seguridad
                        String clean = prereq.trim().replaceAll("^\"|\"$", "");
                        if (!courseGraph.containsCourse(clean)) {
                            // Mostrar aviso en pantalla:
                            outputArea.appendText("⚠ Prerrequisito desconocido: '" + clean +
                                    "' para el curso " + course.getCode() + "\n");
                            // Opcionalmente puedes lanzar excepción o continuar sin ese arco:
                            continue;
                        }
                        courseGraph.addPrerequisite(course.getCode(), clean);
                    }
                }
